...
<div class="card mb-3 input">
  @if($posts[0]->image)
    ...
    <div class="card-body text-center">
      ...
      <a href="/posts/{{$posts[0]->slug}}" class="text-decoration-none btn btn-dark">@lang('posts.baca')</a>
    </div>
  </div>
    ...
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            <div class="card input">
                <div class="position-absolute px-3 py-2" style="background-color :rgba(0,0,0,0.7 green, blue, alpha)"><a href="/posts?category={{$post->category->slug}}">{{ $post->category->name}}</a></div>
                @if($post->image)
                  ...
                  <p>
                    <small class="text-muted">
                        By <a href="authors/{{$post->author->username}}" 
                            class="text-decoration-none text-dark">{{$post->author->name}}</a> {{$post->created_at->diffForHumans()}}
                </small>
                </p>
                  <p class="card-text">{{$post->excerpt}}</p>
                  <a href="/posts/{{$post->slug}}" class="btn btn-dark">@lang('posts.baca')</a>
                </div>
              </div>
        </div>
